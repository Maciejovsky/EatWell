    ...
    if request.user.is_authenticated:
        return render(request, "orders/pizza.html",
                      context={"regular_pizza": RegularPizza.objects.all, "sicillian_pizza": SicilianPizza.objects.all,
                               "toppings": Toppings.objects.all, "number_of_toppings": 3})
    else:
        return redirect("orders:login")
...
def sea_food(request):
    if request.user.is_authenticated:
        return render(request, "orders/sea_food.html", context={"dishes": SeaFood.objects.all})
    else:
        return redirect("orders:login")
...
def kebab(request):
    if request.user.is_authenticated:
        return render(request, "orders/kebab.html", context={"dishes": Kebab.objects.all})
    else:
        return redirect("orders:login")
...
def soup(request):
    if request.user.is_authenticated:
        return render(request, "orders/soup.html", context={"dishes": Soup.objects.all})
    else:
        return redirect("orders:login")
...
def dessert(request):
    if request.user.is_authenticated:
        return render(request, "orders/dessert.html", context={"dishes": Dessert.objects.all})
    else:
        return redirect("orders:login")
...
def drink(request):
    if request.user.is_authenticated:
        return render(request, "orders/drink.html", context={"dishes": Drink.objects.all})
    else:
        return redirect("orders:login")
...
def recommendations(request):
    if request.user.is_authenticated:
        return render(request, "orders/recommendations.html", context={"meals": Meals.objects.all})
    else:
        return redirect("orders:login")
...
def checkout(request):
    if request.method == 'POST':
        ...
        order = UserOrder(username=username, order=list_of_items, price=float(price),
